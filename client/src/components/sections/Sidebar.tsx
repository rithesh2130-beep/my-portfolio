      ...
      <div className="hidden md:flex w-80 lg:w-96 flex-shrink-0 bg-white border-r border-slate-100 flex-col h-screen fixed left-0 top-0 py-8 px-6 z-40 overflow-y-auto">
        ...
        <div className="relative w-full flex flex-col items-center mb-8 shrink-0">
          ...
          <motion.div
            ...
            className="relative w-full max-w-[250px] bg-slate-950 rounded-[24px] border border-slate-800 shadow-xl shadow-slate-950/40 p-5 mt-[-1px] select-none hover:shadow-2xl hover:border-emerald-500/40 transition-all duration-300 group cursor-grab active:cursor-grabbing"
          >
            {/* Slot */}
            <div className="w-10 h-2 bg-slate-900 border border-slate-850 rounded-full mx-auto mb-4" />

            {/* Profile Photo */}
            <div className="relative w-28 h-28 rounded-2xl overflow-hidden border-2 border-slate-800 shadow-inner mx-auto mb-4 group-hover:scale-102 transition-transform duration-300">
              <img
              ...
              <span className="absolute bottom-2 right-2 flex h-3.5 w-3.5">
                ...
                <span className="relative inline-flex h-3.5 w-3.5 rounded-full bg-emerald-500 border-2 border-slate-950" />
              </span>
            </div>

            {/* Name Details */}
            <div className="text-center space-y-1">
              <h2 className="font-display font-extrabold text-white text-lg leading-tight tracking-tight">
                Pandi Rithesh Raja
              </h2>
              <p className="text-[11px] font-bold font-mono text-amber-400 uppercase tracking-wide">
            ...
            <div className="mt-5 pt-3 border-t border-slate-850 flex flex-col items-center">
              <div className="w-full h-6 flex justify-between opacity-70 group-hover:opacity-100 transition-opacity">
                {[2, 4, 1, 3, 2, 4, 1, 2, 3, 1, 4, 2, 3, 1, 2, 4, 1, 3, 2].map((bar, i) => (
                  <div key={i} className="bg-slate-400 h-full" style={{ width: `${bar}px` }} />
                ))}
              </div>
              <span className="text-[8px] font-mono font-bold text-slate-500 mt-1 uppercase tracking-[0.2em]">
                ID: 23HR1A3045
              </span>
            </div>
          </motion.div>
